    ...
    -- Database Creation
    CREATE DATABASE IF NOT EXISTS dz_tabib_medical_directory;
    USE dz_tabib_medical_directory;
   ...
   CREATE TABLE Addresses (
    ...
    -- 9. Doctors Table (Modified to link with Users)
   ...
   CREATE TABLE DoctorAddresses (
    ...
    -- 12. Doctor-Mutuelle Mapping
    CREATE TABLE DoctorMutuelles (
        doctor_id INT,
        mutuelle_id INT,
        PRIMARY KEY (doctor_id, mutuelle_id),
        FOREIGN KEY (doctor_id) REFERENCES Doctors(doctor_id),
        FOREIGN KEY (mutuelle_id) REFERENCES Mutuelles(mutuelle_id)
    );

    -- 13. Doctor-Consultation Type Mapping
    CREATE TABLE DoctorConsultationTypes (
        ...
        FOREIGN KEY (doctor_id) REFERENCES Doctors(doctor_id),
        FOREIGN KEY (consultation_type_id) REFERENCES ConsultationTypes(consultation_type_id)
    );

    -- 14. Patients Table (Modified to link with Users)
...
END //
    ...
    BEGIN
        ...
        FROM Users
        WHERE email = p_email AND password_hash = p_password_hash  = TRUE;
